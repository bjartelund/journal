...
APPNAME=journal
NITRO_STATIC_MODE?=link  # 'link' for dev (fast), 'copy' for release packaging
...
release: rebar3
	# For a release we need actual files, not symlinks, so override mode to copy
	@(NITRO_STATIC_MODE=copy $(MAKE) nitrogen_js_css && \
...
run_dev: rebar3
	@(etc/assemble_config.escript etc etc/autogenerated.config.all && \
	$(REBAR) shell --config etc/autogenerated.config.all --eval "sync:go().")
...
priv/static/nitrogen:
	@(escript copy_static.escript $(NITRO_STATIC_MODE))
